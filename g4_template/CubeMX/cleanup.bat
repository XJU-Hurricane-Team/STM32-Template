...
rem 处理 .ld 文件：移动到 Drivers\CMSIS\Device\ST\STM32G4xx\Source\ld 目录
set "LD_SOURCE_DIR=%PROJECT_DIR%"
set "LD_TARGET_DIR=%ROOT_DIR%CubeMX\Drivers\CMSIS\Device\ST\STM32G4xx\Source\ld\"
set "LD_FILE="
...
if exist "%EIDE_FILE%" (
		...
		powershell -Command "$file = '%EIDE_FILE%'; $ldPath = '.\CubeMX\Drivers\CMSIS\Device\ST\STM32G4xx\Source\ld\%LD_FILE%'; $c = Get-Content -Path $file; $inGcc = $false; for ($i = 0; $i -lt $c.Count; $i++) { if ($c[$i] -match '^\s*GCC:\s*$') { $inGcc = $true; continue }; if ($inGcc -and $c[$i] -match '^\s*scatterFilePath:') { $c[$i] = '        scatterFilePath: ' + $ldPath; $inGcc = $false } }; $c = $c -replace '^\s*toolchain:.*$','    toolchain: GCC'; $c = $c -replace 'Source/arm/','Source/gcc/'; Set-Content -Path $file -Value $c"
)
